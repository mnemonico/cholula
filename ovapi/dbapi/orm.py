...
from ovapi.configuration.assets import DATABASE_USER, DATABASE_PASSWORD, DATABASE_HOST, DATABASE_PORT, DATABASE_NAME
...
db_engine = create_engine(
    url="postgresql://{user}:{password}@{host}:{port}/{name}".format(
        user=DATABASE_USER, password=DATABASE_PASSWORD, host=DATABASE_HOST, port=DATABASE_PORT, name=DATABASE_NAME
    )
)
...
def create_table(*columns, tablename=None, engine=None):
    ...
    table = Table(tablename, meta, *columns)
...
def stmt_insert_update(table, records_to_insert):
    ...
    with session_instance() as o_session:
        for record in records_to_insert:
            stmt_simple_insert = insert(table).values(record)
...
def stmt_delete(table, key, records_to_delete):
    ...
    with session_instance() as o_session:
        for record in records_to_delete:
            stmt_simple_delete = delete(table).where(table.c.line == record[key])
